...
def embed_texts(texts):
    ...
    with torch.no_grad():
        embeddings = model(
            **inputs).last_hidden_state.mean(dim=1)  # Mean pooling
...
# Shape: (num_articles, num_labels)
similarities = cosine_similarity(article_embeddings, label_embeddings)

# Step 4: Predict top-k labels or apply threshold
top_k = 5  # Predict top 5 labels
predictions = torch.topk(torch.tensor(similarities),
                         k=top_k, dim=1).indices.numpy()
print(predictions)
